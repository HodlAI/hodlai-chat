...
const loadInitialFontSize = (): 'small' | 'medium' | 'large' => {
  if (typeof window === 'undefined') return 'medium';
  return (localStorage.getItem('bsc_ai_hub_fontsize') as any) || 'medium';
};
...
export const useStore = create<Store>((set, get) => ({
  ...
  setFontSize: (size) => {
    localStorage.setItem('bsc_ai_hub_fontsize', size);
    set({ fontSize: size });
  },
